...
- **Score sync** (deviation from spec §1, which excluded notation rendering): the score is rendered by OpenSheetMusicDisplay. The score follows the track selection: track 0 is drawn as the top staff and track 1 as the bottom, so unticking a hand hides its staff in the preview and during the session (if the tracks don't line up with the file's staves, every staff is shown). The renderer draws a system's opening clef from the instructions at the *visible* staff index, so hiding the top staff would put a treble clef on the bass staff; `ScoreView` reorders each measure's opening instructions into visible order before rendering, which fixes the glyph without moving any notes. Bars are laid out at equal width (the renderer's `FixedMeasureWidth`, which uses the widest bar for all) so the playhead moves at a steady speed. A red playhead slides continuously between notes, straight through barlines; only at a line break does it head for the barline and then jump to the new system, and the page scrolls to keep the current system in view. It is driven by beat, so the `.mid` and the MusicXML must come from the same MuseScore file with repeats removed. A pickup bar will put the cursor one partial bar off, since the MIDI import counts bars from tick 0. Zoom in the session screen only affects the score.

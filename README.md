...
| `src/LadderView.tsx` | The progression ladder: levels with progress bars, steps grouped by drill with the keys as status-coloured chips (ring = next step, solid accent = loaded in the picker); locked levels shown as one line; click a chip to load it |
...
| `src/ScoreView.tsx` | Renders an attached MusicXML score with OpenSheetMusicDisplay; reads each bar's note area and beat range from the layout, then sweeps its own playhead across bars at constant speed from a frame loop, scrolling to follow |
...
| `src/sessionConfig.ts` | `SessionConfig` and its pure helpers: range start beat, reference notes in range, the opening chord to wait for |
| `src/useSession.ts` | A practice session: optional wait for the opening notes, count-in, recording into a `NoteRecorder`, bar/beat status, passes (one, or repeated with a one-bar gap in loop mode) each scored, auto-stop or Esc |
...
| `src/synth.ts` | Web Audio instrument for hearing the keyboard through the computer and for previews: two oscillators per note, piano-like envelope, sustain pedal, scheduled notes, latency readout |
| `src/playback.ts` | Preview playback: schedules a piece's or drill's reference notes on the synth with lookahead, and gives the playhead its position |
...
- Clocks: MIDI events are on `performance.now()`, clicks on the AudioContext clock; `getOutputTimestamp()` pairs them. A freshly created AudioContext reports zeros from that call for its first ~100 ms, so the metronome re-reads the pairing each scheduler tick until three readings agree within 1 ms, then freezes it (re-reading forever lets its jitter shake the playhead); beat times are always asked for fresh, never cached at session start.
- Preview (▶ on the Pieces and Drills setup cards): plays the selected tracks and bars at the chosen tempo through the synth, with the playhead moving on the score or roll; stops at the end, on Stop, on selecting another piece or drill, or when a session starts. Sound goes to the computer's audio output whatever the header toggle says.
- Play notes through this computer (header toggle, off at each launch): every keyboard event also drives a small synth, so headphones plugged into the Mac carry the piano. It is a synthesised tone, not a piano sample. Latency is the browser's buffer plus the audio device's, typically 10–20 ms wired; the Keyboard check tab shows the measured figure. Bluetooth headphones add 100+ ms and are unusable for this. The keyboard's own headphone jack remains the zero-latency option.
- Wait for the opening notes (on by default, both setup screens): pressing Practice shows the score and waits, naming the first note or chord of the selected bars and tracks. Only those keys start the count-in, all of them within 300 ms, rolled or together; wrong keys do nothing and nothing is graded or penalised until the count-in has run. Esc while waiting cancels without saving anything. The audio context is woken during the Practice click so the count-in triggered by a MIDI note is allowed to sound.
...
- **Score sync** (deviation from spec §1, which excluded notation rendering): the score is rendered by OpenSheetMusicDisplay. The score follows the track selection: track 0 is drawn as the top staff and track 1 as the bottom, so unticking a hand hides its staff in the preview and during the session (if the tracks don't line up with the file's staves, every staff is shown). The renderer draws a system's opening clef from the instructions at the *visible* staff index, so hiding the top staff would put a treble clef on the bass staff; `ScoreView` reorders each measure's opening instructions into visible order before rendering, which fixes the glyph without moving any notes. Bars are laid out at equal width (the renderer's `FixedMeasureWidth`, which uses the widest bar for all). The red playhead sweeps each bar's note area at a constant speed by beat, read from the layout's measure geometry rather than from note positions, so it is steady by construction and close to the noteheads; it jumps to the next system at a line break and the page scrolls to keep the current system in view. It is moved from an animation-frame loop with a CSS transform, never through React state, so a busy frame cannot make it stutter. It is driven by beat, so the `.mid` and the MusicXML must come from the same MuseScore file with repeats removed. A pickup bar will put the cursor one partial bar off, since the MIDI import counts bars from tick 0. Zoom in the session screen only affects the score.
...
- Drills (see `drills-spec.md`): generated on the fly, never stored; the piece id encodes the parameters (`drill:scale:harmonic-minor:F#:both:2:2`) and History rebuilds the piece from it. Everything is in 4/4; scales go up and down with the top note once and the last note held with the longest plain value that fits the bar. Hands together is two tracks an octave apart; four-octave drills start an octave lower. Every drill is repeated back to back until it is at least 12 bars long and graded as one pass, so the exercise is long enough to prove something (a one-octave scale in eighths plays six times, in quarters three times; Hanon is already longer); the number is a constant at the top of `drills.ts`. Keys are spelled conventionally (D♭ major, C♯ minor); modes and pentatonics take the key signature of their parent scale.
- Ladder (see `drills-spec.md`): five levels of steps, each a drill at a tempo. A step is passed when any saved performance of that drill at or above the tempo scored ≥95% pitch with ≥90% of correct notes within ±120 ms (the "close" window; the ±60 ms "on time" figure is informational, since it proved hard to reach even when a scale sounded right); nothing is stored beyond history. A level opens when the one before it is fully passed (Extras opens with Level 3). Locked levels are listed but not expanded; the picker can play anything regardless. Results for a drill say whether that pass met the mark.
...
### If import says the browser could not read the file

Chrome refuses files that a sync or access-control layer still holds, which happens with files created moments ago inside `~/Documents` on a Mac with iCloud or similar syncing. The file itself is fine (seen 2026-09-05: a fresh export in `~/Documents/.../SheetMusic` failed, a copy in `~/Downloads` imported at once, and the previous day's file in the same folder was fine). Export to `~/Downloads`, or wait a minute or two after exporting, then import. Once imported the file is no longer needed; the piece lives in the app.
